...
//!
//! ```
//! cargo test -p zlauder-engine --features ml --test prewarm_parity -- --ignored --nocapture
//! ```

#![cfg(feature = "ml")]

use zlauder_engine::{EngineConfig, MaskEngine, MaskOutcome, MlConfig, Surface};
...
fn engine_with_real_ml() -> MaskEngine {
    ...
    let recognizer =
        zlauder_engine::ml::build_recognizer(&MlConfig::default()).expect("model load");
...
/// stresses, but driven through the whole zlauder engine.
