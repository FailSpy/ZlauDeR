...
/// plane (`/zlauder/reveal`, `/zlauder/config`, the monitor) is only key-gated; a
...
pub fn loopback_guard(bind: &str) -> Result<()> {
    if zlauder_state::is_loopback_bind(bind) {
        return Ok(());
    }
    if std::env::var_os("ZLAUDER_ALLOW_NON_LOOPBACK_BIND").is_some() {
        eprintln!(
            "ZlauDeR: DANGER — binding {bind} exposes the control plane (reveal/config/monitor, \
             ...
             ZLAUDER_ALLOW_NON_LOOPBACK_BIND is set."
        );
        tracing::warn!("binding non-loopback {bind} (ack via ZLAUDER_ALLOW_NON_LOOPBACK_BIND)");
        return Ok(());
    }
    anyhow::bail!(
        "refusing to bind non-loopback address {bind} — this would expose the reveal/config/monitor \
         ...
         you truly intend a reachable proxy, set ZLAUDER_ALLOW_NON_LOOPBACK_BIND=1."
    );
}
...
pub async fn bind_listener(
    ...
    match static_port {
        Some(p) => bind_exact(bind, p).await.map_err(|e| {
            let fault = zlauder_state::classify_bind_error(e.raw_os_error(), e.kind());
            anyhow::anyhow!("{}", fault.message(p, bind))
        }),
        None => {
            if let Some(seed) = zlauder_state::read_rendezvous(project_root)
                .map(|r| r.last_port)
                .filter(|p| *p != 0)
...
mod tests {
    ...
    // ZLAUDER_STATE_DIR / ZLAUDER_ALLOW_NON_LOOPBACK_BIND are process-global; serialize
    ...
    fn loopback_guard_allows_loopback_refuses_public() {
        ...
        unsafe { std::env::remove_var("ZLAUDER_ALLOW_NON_LOOPBACK_BIND") };
        ...
        unsafe { std::env::set_var("ZLAUDER_ALLOW_NON_LOOPBACK_BIND", "1") };
    ...
    async fn ephemeral_binds_a_real_port() {
        let _g = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let dir = std::env::temp_dir().join(format!("zlauder-bind-eph-{}", std::process::id()));
    ...
    async fn static_pin_binds_exact_and_classifies_conflict() {
        let _g = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let dir = std::env::temp_dir().join(format!("zlauder-bind-stat-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        unsafe { std::env::set_var("ZLAUDER_STATE_DIR", &dir) };
    ...
    async fn ephemeral_honors_sticky_seed_then_falls_back() {
        let _g = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let dir = std::env::temp_dir().join(format!("zlauder-bind-stick-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        unsafe { std::env::set_var("ZLAUDER_STATE_DIR", &dir) };
        ...
        zlauder_state::write_rendezvous(&zlauder_state::Rendezvous {
