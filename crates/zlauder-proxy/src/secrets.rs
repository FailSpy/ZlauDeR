...
/// `[ZLAUDER_ADMIN_KEY_<hex>]`), REVEALED on the display path so the model can relay a
...
pub const ADMIN_KEY_SECRET_NAME: &str = zlauder_engine::RESERVED_NONDEREF_SECRET;
...
pub async fn resolve_and_install(
    ...
    for spec in specs {
        ...
        // Compare on the canonical SLUG, not the literal name: `"zlauder admin key"`,
        // `"Zlauder-Admin-Key"`, etc. all slug to `ZLAUDER_ADMIN_KEY` and would collide
...
mod tests {
    ...
    async fn admin_rule_survives_config_secret_install() {
        let name = "ZLAUDER_PROXY_SECRETS_TEST_COEXIST";
        ...
        assert!(out.masked_text.contains("[ZLAUDER_ADMIN_KEY_"));
    ...
    /// A user secret named `zlauder_admin_key` is rejected regardless of operator
    /// (here `hash`), and a REQUIRED one holds the gate closed.
    #[tokio::test]
    async fn reserved_admin_name_rejected() {
        let env = "ZLAUDER_PROXY_SECRETS_TEST_RESERVED";
        unsafe { std::env::set_var(env, "whatevervalue123") };
        let engine = MaskEngine::new(EngineConfig::default()).unwrap();
        let registry = zlauder_secrets::default_registry(None);
        let specs = vec![spec("zlauder_admin_key", env, Some("hash"), true)];
        let (status, ok) = resolve_and_install(&specs, &engine, &registry, TEST_ADMIN_KEY).await;
        ...
        assert!(
            status
                .entries
                .iter()
                .any(|e| e.name.eq_ignore_ascii_case("zlauder_admin_key") && !e.resolved),
            "the rejected reserved-name secret should be recorded as unresolved"
        );
    }

    /// A slug-VARIANT of the reserved name ("zlauder admin key" → same ZLAUDER_ADMIN_KEY
    ...
    async fn reserved_admin_slug_variant_rejected() {
        let env = "ZLAUDER_PROXY_SECRETS_TEST_SLUGVAR";
        unsafe { std::env::set_var(env, "whatevervalue456") };
        let engine = MaskEngine::new(EngineConfig::default()).unwrap();
        let registry = zlauder_secrets::default_registry(None);
        let specs = vec![spec("zlauder admin key", env, Some("broker"), true)];
        let (status, ok) = resolve_and_install(&specs, &engine, &registry, TEST_ADMIN_KEY).await;
        unsafe { std::env::remove_var(env) };
