#!/usr/bin/env bash
# zlauder UserPromptSubmit hook: the fail-CLOSED first-session INTAKE GATE.
#
...
# state — not plumbed, already routed, opted out, or ZLAUDER_NO_INTAKE_GATE set — emits nothing
# (the prompt proceeds).
#
# The whole decision lives in `zlauder-hooks user-prompt-submit` (fast LOCAL reads only — no
...
# Binaries unavailable ⇒ no zlauder installed ⇒ no masking promise ⇒ allow (emit nothing).
set -euo pipefail
...
# Binaries unavailable ⇒ emit nothing (the prompt proceeds; no zlauder = nothing to gate).
...
"$ZLAUDER_HOOKS_BIN" user-prompt-submit || true
